...
export class HDEventExecutorService {
  ...
  execute(meta: any, eventTreeNode: any) {
    ...
    if (eventTreeNode.type === EVENT_TYPE.ALERT) {
      ...
      if (
        eventTreeNode.targetEvent !== undefined &&
        eventTreeNode.targetEvent.length > 0
      ) {
  ...
  executeMessageAlert(meta: any, eventNode: any) {
    ...
    if (
      eventNode.targetEvent !== undefined &&
      eventNode.targetEvent.length > 0
    ) {
  ...
  async executeRefreshElement(meta: any, eventNode: any) {
    ...
    Promise.all(promises).then((resolveData) => {
      if (
        eventNode.targetEvent !== undefined &&
        eventNode.targetEvent.length > 0
      ) {
  ...
  private executeEventAfterMultipleHandler(
    meta: any,
    eventNode: any,
    edges: any,
    ...
    const edge = edges.find(
      (edge: any) =>
        edge.source === eventNode.id && edge.sourceHandle === confirmationType
    );
    if (edge) {
      const nextEventId = edge.target;
      const nextEventNode = eventNode.targetEvent.find(
        (tEvent: any) => tEvent.id == nextEventId
      );
      if (nextEventNode) {
